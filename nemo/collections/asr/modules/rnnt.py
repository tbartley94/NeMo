...
class HainanJoint(rnnt_abstract.AbstractRNNTJoint, Exportable, AdapterModuleMixin):
    """A Recurrent Neural Network Transducer Joint Network (RNN-T Joint Network).
    An RNN-T Joint network, comprised of a feedforward model.
    ...
    def forward(
        ...
        res = self.joint_net(encoder_outputs)  # [B, T, V + 1]
        B, T, V = res.shape
        res = torch.reshape(res, [B, T, 1, V])
    ...
    @property
    def fuse_loss_wer(self):
        return False # self._fuse_loss_wer
