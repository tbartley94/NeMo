...
#            print("SKIP IS", skip)
    ...
    def _init_t_matrix(self, t_probs):
        U = t_probs.shape[0]
        t_matrix =  torch.tensor([[0.0 for _ in range(U+1)] for _ in range(U+1)], device=t_probs.device)
        for i in range(U):
            t_matrix = self._update_t_matrix(t_matrix, t_probs[i], i)
        return t_matrix
    
    def _init_t_matrix_dict(self, t_probs):
        U = t_probs.shape[0]
        t_matrix =  {idx: {} for idx in range(U+1)}
        for i in range(U):
            t_matrix = self._update_t_matrix_dict(t_matrix, t_probs[i], i)
        return t_matrix
    ...
    # assumes per time step logits
    def _update_t_matrix_dict(self, t_matrix, probs, t):
        U = len(t_matrix)
        for idx, n in enumerate(self.durations):
            target = t + n
            if n == 0:
    ...
    def _clean_backtrack(self, k_t, backtrack):
        skip = []
        shared_backtrack = defaultdict(list)
        for idx in reversed(range(len(backtrack))):
            skip.append(idx)

            prev = backtrack[idx]
            while prev > 0 and k_t[prev] == self._blank_index:
                skip.append(prev)
                prev = backtrack[prev]

            for s in skip:
                backtrack[s] = prev
                if s == len(k_t): # don't need sentinel token
                    continue
                shared_backtrack[prev].append(s)
            skip = []

        return backtrack, shared_backtrack
    ...
    def _markov_decode(
        ...
        for t in range(1, U):
            ...
            prev_states = [_ for _ in range(start, t)]

            a_prob = alphas[prev_states]
            e_prob = e_matrix[t]
            t_prob = torch.stack([t_matrix[s][t] for s in prev_states], dim=0)
    ...
    def _iter_markov_decode(
        ...
        t_matrix = self._init_t_matrix_dict(t_probs)
        backtrack, alphas = self._markov_decode(e_matrix, t_matrix)

        # we purge blanks to make decoding on the loop quicker
        backtrack = backtrack.tolist()
        backtrack, shared_backtrack = self._clean_backtrack(k_t, backtrack)

        for _ in range(loop_count):
            new_k_t = torch.tensor([-1 for _ in range(U)], device=x.device)
            new_e_matrix = torch.tensor([0.0 for _ in range(U+1)], device=x.device)
            # updates
            dec_states = [None for _ in range(U)]
            start_states = sorted(shared_backtrack.keys())
            for s in start_states:
                targets = shared_backtrack[s]
                T = len(targets)
                last_token, last_state = None, None
                if s != -1 and last_token != self._blank_index: # start state check
                    last_token = k_t[s].view(1, 1)
                    last_token = last_token.expand(T, -1)

                    last_h, last_c = dec_states[s]
                    last_h = last_h.repeat(1, T, 1)
                    last_c = last_c.repeat(1, T, 1)
                ...
                logits = logits.view([T, -1])
                v_prob = logits[:,:-len(self.durations)].log_softmax(dim=-1)

                # update
                for idx, t in enumerate(targets):
                    last_h, last_c = last_state
                    last_h = last_h[:,idx,:].reshape(last_h.shape[0], 1, -1)
                    last_c = last_c[:,idx,:].reshape(last_c.shape[0], 1, -1)
                    dec_states[t] = last_h, last_c

                    new_e_matrix[t] += v_prob.max(dim=-1)[0][idx]
                    new_k_t[t] = v_prob.argmax(dim=-1)[idx].item()
                del f, g
