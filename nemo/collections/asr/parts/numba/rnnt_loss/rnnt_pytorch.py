...
__all__ = ['rnnt_loss', 'RNNTLossNumba', 'MultiblankRNNTLossNumba', 'TDTLossNumba', 'HainanLossNumba']
...
class _HainanNumba(Function):
    ...
    def forward(
        ...
        if is_cuda:
            loss_func = rnnt.hainan_loss_gpu
        else:
...
class HainanLossNumba(Module):
    ...
    def __init__(
        ...
        super(HainanLossNumba, self).__init__()
        self.blank = blank
        self.durations = durations if durations is not None else []
        self.fastemit_lambda = fastemit_lambda
        self.clamp = float(clamp) if clamp > 0 else 0.0
        self.reduction = reduction
        self.loss = _HainanNumba.apply
        self.sigma = sigma
        self.omega = omega
